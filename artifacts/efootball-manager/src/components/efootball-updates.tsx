...
export function EFootballUpdates() {
  ...
  if (isLoading) {
    return (
      <section className="glass-card rounded-2xl p-5 sm:p-6 animate-pulse">
        <div className="h-4 w-48 rounded bg-white/10" />
        <div className="mt-5 h-48 rounded-xl bg-white/5" />
      </section>
    );
  }
  ...
  if (error || !data || !pack || !player) {
    return (
      <section className="glass-card rounded-2xl border border-primary/20 p-5 sm:p-6">
        <div className="flex items-center gap-3">
          <BrainCircuit className="h-5 w-5 text-primary" />
          <div>
            <h2 className="font-display text-lg font-bold text-white">eFootball Pack Watch</h2>
            <p className="text-sm text-zinc-500">{error || "No pack updates are available right now."}</p>
          </div>
        </div>
        <button
          onClick={() => void loadUpdates()}
          className="mt-4 inline-flex items-center gap-2 border border-primary/30 px-3 py-2 text-xs font-bold uppercase tracking-wider text-primary hover:bg-primary/10"
        >
  ...
  return (
    <section className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#151820]/90 p-5 shadow-[0_18px_50px_rgba(0,0,0,0.25)] sm:p-6">
      ...
      <div className="relative mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
        <div className="relative flex min-h-[245px] overflow-hidden border border-white/10 bg-gradient-to-br from-[#232733] to-[#101217]">
          ...
          <div className="relative flex w-full items-center justify-between gap-4 p-5 sm:p-7">
            <div className="max-w-[58%] sm:max-w-[52%]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs uppercase tracking-widest text-zinc-500">{pack.name}</span>
              </div>
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="mt-5 inline-flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-primary hover:text-white"
              >
            ...
            <img
              ...
              className="absolute bottom-0 right-2 h-[190px] w-auto object-contain drop-shadow-[0_15px_20px_rgba(0,0,0,0.45)] sm:right-10 sm:h-[230px]"
            />
          </div>
        </div>

        <div className="border border-primary/30 bg-primary/[0.07] p-5 sm:p-6">
          ...
          <div className="mt-4 border-l-2 border-primary/50 pl-3 text-xs leading-relaxed text-zinc-500">
            <strong className="text-zinc-300">Why this build:</strong> {player.training.reason}
          </div>
      ...
      {playerCount > 1 && (
        <div className="relative mt-4 flex items-center gap-3">
          <button
            ...
            className="border border-white/10 p-2 text-zinc-400 hover:border-primary/40 hover:text-primary"
      ...
      <div className="relative mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-white/8 pt-4 text-[10px] text-zinc-500">
        <span>{data.source.note}</span>
        <span className="flex items-center gap-3">
          <a href={pack.packUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-primary hover:text-white">
            EFHub pack <ExternalLink className="h-3 w-3" />
          </a>
          <a href={data.source.officialUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-zinc-400 hover:text-white">
